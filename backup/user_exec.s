...
;   rsi-interp PT_LOAD base virt address
;
; Out:
;   rax-interp e_entry
;
...
load_interp:
    ...
    mov     [rbp-0x10], rdi
    mov     [rbp-0x18], rsi           ; base load address

    mov     rax, [rdi + 0x18]         ; e_entry
    add     rax, rsi                  ; entry point + base load address
...
.begin:
    ...
    ; e_type ET_DYN
    mov     r8, [rbp-0x18]
    add     r12, r8                 ; add base load address to virtaddr

    mov     r8, r12
    and     r8, 0xfff               ; page alignment remaining
    add     r13, r8                 ; increase size due to v_addr alignment
...
update_auxv:
    ...
    mov     rsi, 7                  ; AT_BASE
    mov     rdx, rcx                ; interp base address
    call    set_auxv_val
